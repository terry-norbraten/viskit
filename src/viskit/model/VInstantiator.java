...
public abstract class VInstantiator
{
  ...
  public static class Constr extends VInstantiator
  {
    ...
    public Constr(List<Object> params, String type) {
        ...
        while(li.hasNext()) {
            ...
            if ( o instanceof TerminalParameter ) { // check if caller is sending assembly param types
                s1 = ((TerminalParameter)o).getType();  if(viskit.Vstatics.debug)System.out.print("\tAssembly TerminalParameter");
            } else if ( o instanceof MultiParameter ) {
                s1 = ((MultiParameter)o).getType(); if(viskit.Vstatics.debug)System.out.print("\tAssembly MultiParameter");
            } else if ( o instanceof FactoryParameter ) {
                s1 = ((FactoryParameter)o).getType();   if(viskit.Vstatics.debug)System.out.print("\tAssembly FactoryParameter");              
            } else if (o instanceof Parameter) { // from InstantiationPanel, this could also be an eventgraph param type?
                s1 = ((Parameter)o).getType(); if(viskit.Vstatics.debug)System.out.print("\tEventGraph Parameter");
        ...
        if (eparams != null) {
            ...
            if (eparams[indx] != null) { 
                ...
                if ( args != null ) for ( int j = 0; j < eparams[indx].size(); j++ ) {//for ( int j = 0; j < args.size(); j++ ) {
                    if (viskit.Vstatics.debug) System.out.println("setting name "+((Parameter)eparams[indx].get(j)).getName());
    ...
    public Constr(String type, List<Object> args, List names)
    {
      this(type, args);
      for(int i=0;i<args.size();i++) {
    ...
    List<Object> buildInstantiators(List<Object> assemblyParameters) {
        ...
        while (it.hasNext()) {
            ...
            if (o instanceof TerminalParameter) {
                ...
                if (viskit.Vstatics.debug) System.out.println("Conversion from "+((Parameter)o).getType()); // 
      ...
      //while ( indx <= parameters.length - 1 && !found ) {
          ...
          if ( parameters[i].size() == args.size() ) {
              ...
              for ( int j = 0; j < args.size(); j++ ) {
                  ...
                  if (viskit.Vstatics.debug) System.out.print("touching "+Vstatics.convertClassName(((Parameter)(parameters[i].get(j))).getType())+" "+((VInstantiator)(args.get(j))).getType());
                  String pType = Vstatics.convertClassName(((Parameter)(parameters[i].get(j))).getType());
                  ...
                  ((VInstantiator)(args.get(j))).setName(
                        ((Parameter)(parameters[i].get(j))).getName()
                  );
                  if (viskit.Vstatics.debug) System.out.println(" to "+((Parameter)(parameters[i].get(j))).getName());
  ...
  public static class Array extends VInstantiator
  {
    ...
    public VInstantiator vcopy()
    {
      Vector<VInstantiator> lis = new Vector<VInstantiator>();
  ...
  public static class Factory extends VInstantiator
  {
    ...
    public void setParams(List<Object> p) {this.params = p;}
